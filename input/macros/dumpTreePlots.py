...
def main(argv=None):
    ...
    inputFile=TFile("SCTPCtest.root")
    ...
    with open("SCTPCtest.p", "wb") as pickle_file:
        pickle.dump(segments, pickle_file)
    print("dE: ", segments["dE"])
    # plot
    f = -1
    count = 0
    E = ['dEdx','xyz','dx']
    while f != 0 and count<len(E):
        isPlt = True # is there a figure open
        makePlot(E[count], segments, isPlt)
        f = input("Enter 0 to end loop. 1 to continue to next plot: ")
        if isPlt==True:
            plt.close()
        count = count + 1

# for adding labels to plots. Update when more plots are made
def makePlot(p, Dict,isPlt):
    if p == "dEdx":
        isPlt = True
        plt.figure()
        plt.hist(Dict[p], bins = 1000)
        plt.xlabel(p)
        #plt.ylabel('')
        plt.xlim(0.0,600.0)
        plt.title('dE/dx (MeV/cm)')
        plt.show(block=False)
    if p == "xyz":
        isPlt = True
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(Dict['x'],Dict['y'],Dict['z'])
        ax.set_xlabel('x (cm)')
        ax.set_ylabel('y (cm)')
        ax.set_zlabel('z (cm)')
        ax.set_title('Tracks in 3D')
        plt.show(block=False)
    if p == "dx":
        isPlt = True
        plt.figure()
